########## exp setup ##########
cd /home/students/lmu_proj/surgery-scene-graphs
...
echo "[Qwen3] Precomputing features for ${dataset_path} using ${QWEN3_MODEL_ID} (no quant)"
/home/students/.pixi/bin/pixi run -q python - <<'PY'
from pathlib import Path
import sys
sys.path.append('.')
from qwen_vl_qwen3 import get_patched_qwen3
from generate_qwen_features_qwen3 import extract_qwen3_features

clip_dir = Path("data/cholecseg8k/preprocessed_ssg/video27/video27_00480")
model, processor = get_patched_qwen3(model_path="Qwen/Qwen3-VL-8B-Instruct", use_bnb_4bit=False, use_bnb_8bit=False)
extract_qwen3_features(clip_dir, model, processor)
print("[Qwen3] Feature precomputation completed for:", clip_dir)
PY

########## time-agnostic language field ##########
export language_feature_hiddendim=${clip_feat_dim}
export use_discrete_lang_f=f
/home/students/.pixi/bin/pixi run -q python train.py -s  ${dataset_path} --port 6031 --expname ${exp_name} --configs arguments/cholecseg8k/no_tv.py --include_feature \
...
/home/students/.pixi/bin/pixi run -q python render.py -s  ${dataset_path} --language_features_name ${language_feature_name} --model_path output/${exp_name} \
...
export language_feature_hiddendim=${clip_feat_dim}
/home/students/.pixi/bin/pixi run -q python extract_graph_maskfilter.py \
  -s  ${dataset_path} \
