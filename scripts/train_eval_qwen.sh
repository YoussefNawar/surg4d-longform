...
python train.py -s data/hypernerf/${dataset_name} --port 6021 --expname ${clip_exp} --configs arguments/hypernerf/chicken.py --include_feature \
  --language_features_name ${clip_feat_name} --feature_level ${level} \
  --coarse_base_iterations 0 --coarse_lang_iterations 0 --fine_base_iterations 0 --fine_lang_iterations ${clip_lang_iters} \
  --no_dlang 1 --start_checkpoint output/${base_exp}/point_cloud/fine-base_iteration_${base_fine_iters}/chkpnt_fine-base_${base_fine_iters}.pth \
...
########## 4) Cluster based on CLIP features ##########
echo "===== Cluster based on CLIP features"
# Ensure Python-side modules size to CLIP F when loading the CLIP model
export language_feature_hiddendim=${clip_feat_dim}
python cluster.py -s data/hypernerf/${dataset_name} --language_features_name ${clip_feat_name} --model_path output/${clip_exp} \
  ...
  --num_views 5 --autoencoder_ckpt_path autoencoder/ckpt/${dataset_name}_clip/best_ckpt.pth