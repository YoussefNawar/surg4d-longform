...
def compute_spatial_metrics(cfg: DictConfig):
    ...
    for clip in cfg.clips:
        ...
        for method_name, method_preds in methods_preds.items():
            ...
            for annotation in annotations:
                ...
                if (
                    pred_item
                    and "predicted" in pred_item
                    and pred_item["predicted"] is not None
                ):
                ...
                if not parse_success:
                    method_all_parse_failures[method_name].append(
                        f"{clip_name}:{query_id}"
                    )
                ...
                query_results.append(
                    {
                        ...
                        "predicted_pixel": (
                            [px, py] if px is not None and py is not None else None
                        ),
                        "l2_distance": (
                            round(l2_distance, 2)
                            if l2_distance != float("inf")
                            else None
                        ),
                    }
                )
            ...
            clip_results[method_name] = {
                ...
                "mean_l2_distance": (
                    round(float(np.mean(method_distances)), 2)
                    if method_distances
                    else None
                ),
                "std_l2_distance": (
                    round(float(np.std(method_distances)), 2)
                    if method_distances
                    else None
                ),
            }
    ...
    for method_name in method_all_distances.keys():
        ...
        aggregated[method_name] = {
            ...
            "mean_l2_distance": (
                round(float(np.mean(distances_list)), 2) if distances_list else None
            ),
            "std_l2_distance": (
                round(float(np.std(distances_list)), 2) if distances_list else None
            ),
...
def compute_temporal_metrics(cfg: DictConfig):
    ...
    for clip in cfg.clips:
        clip_name = str(clip.name)
        labels_path = (
            Path(cfg.compute_metrics.annotations_root)
            / "temporal"
            / f"{clip_name}.json"
        )
        pred_path = pred_root / f"{clip_name}.json"
        ...
        for method_name, method_preds in methods_preds.items():
            ...
            for annotation in annotations:
                ...
                if query_type == "pit":
                    ...
                    if pred_item and pred_item.get("predicted"):
                        ...
                        try:
                            iou = compute_temporal_iou(
                                gt_ranges, pred_ranges, cfg.compute_metrics.n_timesteps
                            )
                        except (TypeError, ValueError):
                            pred_ranges = None
                            iou = 0.0
                            method_all_parse_failures[method_name].append(
                                f"{clip_name}:range:{query_id}"
                            )
                    else:
                        # No prediction or parsing failed
                        pred_ranges = None
                    ...
                    raise ValueError(
                        f"Unsupported query type for {clip_name} {query_id}: {query_type}"
                    )
            ...
            clip_results[method_name] = {
                ...
                "num_queries": len(query_results),
                "mean_absolute_error": (
                    round(float(np.mean(method_errors)), 2) if method_errors else None
                ),
                "mean_iou": (
                    round(float(np.mean(method_ious)), 2) if method_ious else None
                ),
            }
        ...
        for method_name, method_preds in methods_preds.items():
            ...
            for annotation in annotations:
                ...
                if pred_item and pred_item.get("predicted") is not None:
                    ...
                    if (
                        ...
                        try:
                            ...
                            l1_per_axis_mean = float(
                                (x_abs_error + y_abs_error + z_abs_error) / 3.0
                            )
                            parse_success = True
                        except (TypeError, ValueError):
                ...
                mean_non_neutral_axis_l1_distance = (
                    float(np.mean(non_neutral_axis_errors))
                ...
                method_all_non_neutral_axis_errors[method_name].extend(
                    non_neutral_axis_errors
                )
                ...
                method_all_axis_errors_by_gt_value[method_name]["x"][gx_int].append(
                    x_abs_error
                )
                method_all_axis_errors_by_gt_value[method_name]["y"][gy_int].append(
                    y_abs_error
                )
                method_all_axis_errors_by_gt_value[method_name]["z"][gz_int].append(
                    z_abs_error
                )
                ...
                query_results.append(
                    {
                        ...
                        "predicted_direction": (
                            {"x": px, "y": py, "z": pz} if px is not None else None
                        ),
                        "axis_l1_distances": {
            ...
            clip_results[method_name] = {
                ...
                "mean_axis_l1_distance_per_axis_by_gt_value": {
                    axis_name: {
                        str(gt_value): (
                            round(float(np.mean(gt_value_errors)), 3)
                            if gt_value_errors
                            else None
                        )
    ...
    for method_name, errors in method_all_errors.items():
        ...
        aggregated[method_name] = {
            ...
            "mean_axis_l1_distance_per_axis": {
                ...
                for axis_name, axis_errors in method_all_axis_errors[
                    method_name
                ].items()
            },
            "mean_axis_l1_distance_per_axis_by_gt_value": {
                ...
                for axis_name, by_gt in method_all_axis_errors_by_gt_value[
                    method_name
                ].items()
            },
...
def compute_temporal_iou(
    gt_ranges: list[list[int]], pred_ranges: list[list[int]], max_timestep: int
) -> float:
