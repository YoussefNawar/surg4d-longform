...
def load_cotracker_data(
    clip_dir: Path, cotracker_subdir: str
    ...
    data = {
        "control_points_3d": torch.from_numpy(
            np.load(cotracker_dir / "control_points_3d.npy")
        ),
        "gaussian_control_point_indices": torch.from_numpy(
            np.load(cotracker_dir / "point_control_point_indices.npy")
        ),
        "gaussian_control_point_weights": torch.from_numpy(
            np.load(cotracker_dir / "point_control_point_weights.npy")
        ),
        "gaussian_positions_precomputed": torch.from_numpy(
            np.load(cotracker_dir / "point_positions_precomputed.npy")
        ),
    }
...
def get_gaussian_positions_at_time(
    ...
    if n_control_driven > 0:
        positions[control_driven_mask] = gaussian_positions_precomputed[
            time_idx, :n_control_driven
        ]
