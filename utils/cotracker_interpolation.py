...
def interpolate_gaussian_positions_from_control_points(
    ...
    control_points_selected = control_points_3d[
        :, control_point_indices
    ]  # (T, N_gaussians, K, 3)
    weights_expanded = control_point_weights.unsqueeze(0).expand(
        T, -1, -1
    )  # (T, N_gaussians, K)

    # Weighted sum: (T, N_gaussians, K, 3) * (T, N_gaussians, K, 1) -> (T, N_gaussians, 3)
    gaussian_positions = (control_points_selected * weights_expanded.unsqueeze(-1)).sum(
        dim=2
    )
    ...
    sampled_gaussians = gaussian_positions[
        :, random_indices, :
    ]  # (T, N_sampled_gaussians, 3)

    # Get the control points for each sampled Gaussian
    sampled_control_point_indices = control_point_indices[
        random_indices, :
    ]  # (N_sampled_gaussians, K)
    sampled_control_points = control_points_3d[
        :, sampled_control_point_indices
    ]  # (T, N_sampled_gaussians, K, 3)
    ...
    colors_gaussians = torch.rand(
        n_samples, 3, device=device
    )  # (N_sampled_gaussians, 3)
    colors_control_points = colors_gaussians.repeat_interleave(
        K, dim=0
    )  # (N_sampled_gaussians * K, 3)
