...
def track_control_points(
    ...
    predictor = CoTrackerPredictor(
        checkpoint="checkpoints/cotracker3_scaled_offline.pth"
    )
    predictor.to("cuda")
    ...
    with torch.no_grad():
        for query_frame in query_frames:
            ...
            frame_indices = torch.full(
                (n_points_per_frame,), query_frame, dtype=torch.float32
            )
            ...
            tracks, visibility = predictor(
                images_torch, queries=queries, backward_tracking=backward
            )
            ...
            logger.info(
                f"Query frame {query_frame}: tracked {n_points_per_frame} random points"
            )
    ...
    visualizer.visualize(
        video=images_torch * 255.0,
        tracks=tracks,
        visibility=visibility,
        filename="cotracker3_visualization_random",
    )
...
def lift_control_points_to_3d(
    ...
    depth_all, _, _, intrinsics_list, w2c_list, c2w_list = load_da3_geometry(
        geometry_npz_path
    )
    assert (
        depth_all.shape[0] == T
    ), f"Mismatch: {depth_all.shape[0]} depth frames, {T} frames"
    assert (
        len(intrinsics_list) == T
    ), f"Mismatch: {len(intrinsics_list)} camera parameters, {T} frames"
    ...
    logger.info(
        f"Lifting {N_grid} control points to 3D world coordinates for {T} frames..."
    )
    ...
    logger.info(
        f"Resolution: original ({orig_W}x{orig_H}) -> processed ({processed_W}x{processed_H}), scale=({scale_x:.3f}, {scale_y:.3f})"
    )
    ...
    was_filled = torch.zeros(
        (T, N_grid), device="cuda", dtype=torch.bool
    )  # Track which were forward/backward filled
    ...
    if fill_occlusions:
        logger.info(
            "Forward-filling with camera-aware depth propagation, depth jump detection, and color forwarding..."
        )
    else:
    ...
    for t in range(T):
        ...
        if valid_at_t.any():
            ...
            last_valid_world_pos = torch.where(
                valid_at_t.unsqueeze(-1), world_pos, last_valid_world_pos
            )
            last_valid_colors = torch.where(
                valid_at_t.unsqueeze(-1), current_colors, last_valid_colors
            )
            ...
            filled_colors[t] = torch.where(
                valid_at_t.unsqueeze(-1), current_colors, filled_colors[t]
            )
        ...
        if fill_occlusions:
            ...
            if needs_fill.any():
                ...
                filled_depths[t] = torch.where(
                    needs_fill, reprojected_depth, filled_depths[t]
                )
                filled_colors[t] = torch.where(
                    needs_fill.unsqueeze(-1), last_valid_colors, filled_colors[t]
                )
                was_filled[t] = was_filled[t] | needs_fill

    if killed_count > 0:
        logger.info(
            f"Killed {killed_count} points due to depth jumps (threshold={depth_jump_threshold}m)"
        )
    ...
    if fill_occlusions:
        ...
        next_valid_world_pos = torch.zeros(
            (N_grid, 3), device="cuda", dtype=torch.float32
        )
        ...
        for t in range(T - 1, -1, -1):
            ...
            if valid_at_t.any():
                world_pos = unproject_to_world(points_2d, sampled_depth, K, c2w)
                next_valid_world_pos = torch.where(
                    valid_at_t.unsqueeze(-1), world_pos, next_valid_world_pos
                )
                next_valid_colors = torch.where(
                    valid_at_t.unsqueeze(-1), current_colors, next_valid_colors
                )
                has_valid_backward = has_valid_backward | valid_at_t

            # For points that still need fill (weren't filled in forward pass)
            still_needs_fill = (
                (filled_depths[t] == 0) & has_valid_backward & ~killed_points
            )
            if still_needs_fill.any():
                ...
                filled_depths[t] = torch.where(
                    still_needs_fill, reprojected_depth, filled_depths[t]
                )
                filled_colors[t] = torch.where(
                    still_needs_fill.unsqueeze(-1), next_valid_colors, filled_colors[t]
                )
    ...
    if fill_occlusions:
        logger.info(
            f"Filtered {N_grid} control points to {N_valid} permanently valid points "
            f"({N_grid - N_valid} removed: {killed_count} killed, {N_grid - N_valid - killed_count} never valid)"
        )
    else:
        logger.info(
            f"Filtered {N_grid} control points to {N_valid} always-directly-valid points "
...
def _compute_processed_resolution(
    orig_w: int, orig_h: int, patch_size: int = 14
) -> Tuple[int, int]:
    """
...
def compute_gaussian_control_point_associations(
    ...
    # Load DA3 geometry and camera parameters
    depth_all, conf_all, _, intrinsics_list, _, c2w_list = load_da3_geometry(
        geometry_npz_path
    )
    ...
    logger.info(
        f"Depth map at processed resolution ({processed_w}x{processed_h}), images at original ({W_original}x{H_original})"
    )

    # Compute confidence threshold from DA3 confidence maps
    conf_thresh = np.percentile(conf_all, conf_thresh_percentile)
    logger.info(
        f"Computed confidence threshold: {conf_thresh} (percentile={conf_thresh_percentile})"
    )
    ...
    if instance_mask is not None:
        ...
        if (processed_w, processed_h) != (W_orig_mask, H_orig_mask):
            instance_mask_np = cv2.resize(
                instance_mask_np.astype(np.float32),
            ...
            logger.info(
                f"Resized instance mask from ({W_orig_mask}, {H_orig_mask}) to processed resolution ({processed_w}, {processed_h})"
            )
    ...
    if (processed_w, processed_h) != (W_original, H_original):
        conf_map = cv2.resize(
            conf_map, (processed_w, processed_h), interpolation=cv2.INTER_NEAREST
        )
    if pixel_stride > 1:
    ...
    logger.info(
        f"Computing associations for {N_pixels} valid pixels (after pixel_stride={pixel_stride} subsampling, GPU-accelerated, 3D distances)..."
    )
    ...
    depth_map_torch = (
        torch.from_numpy(depth_map).float().to(device)
    )  # (H, W) subsampled
    depths = depth_map_torch[
        torch.from_numpy(y_coords).long().to(device),
        torch.from_numpy(x_coords).long().to(device),
    ]  # (N_pixels,)
    ...
    distances_matrix = torch.cdist(
        gaussian_positions_3d, control_points_torch
    )  # (N_pixels, N_grid)

    # Find K nearest neighbors using topk
    distances, indices = torch.topk(
        distances_matrix, k=k_neighbors, dim=1, largest=False
    )
    ...
    if instance_mask is not None:
        ...
        instance_mask_torch = instance_mask.int().to(
            device
        )  # (H, W) in subsampled space
        ...
        control_point_coords = control_points_2d_init.long().to(
            device
        )  # (N_grid, 2) in original image space
        ...
        if (processed_w, processed_h) != (W_original, H_original):
            ...
            control_point_coords_processed[:, 0] = (
                control_point_coords_processed[:, 0] * scale_w
            )
            control_point_coords_processed[:, 1] = (
                control_point_coords_processed[:, 1] * scale_h
            )
        ...
        control_point_coords_subsampled[:, 0] = torch.clamp(
            control_point_coords_subsampled[:, 0], 0, W - 1
        )
        control_point_coords_subsampled[:, 1] = torch.clamp(
            control_point_coords_subsampled[:, 1], 0, H - 1
        )
        ...
        pixel_instance_ids = instance_mask_torch[
            y_coords_torch, x_coords_torch
        ]  # (N_pixels,)
        # Store for returning
        ...
        pixel_instance_ids_expanded = pixel_instance_ids.unsqueeze(1).expand(
            -1, k_neighbors
        )  # (N_pixels, K)
        ...
        gaussian_instance_ids = torch.full(
            (N_pixels,), -1, dtype=torch.long, device=device
        )
    ...
    pixel_to_gaussian_map = torch.full(
        (H_original, W_original), -1, dtype=torch.long, device=device
    )
    pixel_coords_torch_final = torch.stack(
        ...
        dim=-1,
    )  # (N_pixels, 2) in original image space
    ...
    logger.info(
        f"Computed associations: {N_pixels} pixels -> {k_neighbors} control points each"
    )
